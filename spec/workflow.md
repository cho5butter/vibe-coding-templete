...
4. `.github/workflows/quality-gate.yml` のセットアップステップをプロジェクトに合わせて編集する
...
5. **クロスAIレビューを実施する**（詳細は後述）

## クロスAIレビュー

### 目的

各計画セッション終了時に、**開発に使用したAIとは別のAI**で変更箇所のレビューを実施する。これにより、単一AIの盲点を補い、品質を向上させる。

### レビュー優先順位

| 開発AI | レビューAI（第一優先） | レビューAI（第二優先） |
|--------|----------------------|----------------------|
| Claude Code | **Codex** | Claude（別セッション） |
| Codex | **Claude Code CLI** | Codex（別セッション） |
| GitHub Copilot | **Codex** | Claude Code CLI |

### レビュー実行手順

1. 計画セッションの実装・品質ゲート・コミットが完了した後に実施する
2. `bash scripts/cross-review.sh` を実行してレビュー用の差分サマリーを生成する
3. レビューAIに差分サマリーと以下の観点でレビューを依頼する:
   - **要件との整合性**: `spec/requirements.md` の要件・受け入れ基準を満たしているか
   - **設計との整合性**: `spec/design.md` の設計方針に沿っているか
   - **コード品質**: 可読性、保守性、命名規約
   - **セキュリティ**: OWASP Top 10 の観点での脆弱性チェック
   - **テストの十分性**: エッジケース・異常系のカバレッジ
   - **パフォーマンス**: 明らかな非効率がないか
4. レビューで指摘された問題は、次の計画セッションの冒頭で対応する
...
- レビュー結果は記録として残す（コミットログやPRコメント等）
- **重大な問題**: 即座に修正し、再度品質ゲートを通す
- **軽微な改善提案**: 次の計画セッションの冒頭で対応を検討する
- **見解の相違**: ユーザーに判断を仰ぐ
